...
async function probe() {
  ...
  probes = {
    ...
    processes: {
      supervisor: /supervise\.sh/.test(procs),
...
function supervisor() {
  ...
  if (startLine) {
    ...
    else { const d = new Date(start); d.setHours(+until[1], +until[2], 0, 0); deadline = d.getTime(); }
  }
  ...
  for (const l of sinceStart) {
    const m = l.match(/round (v\d+): training on (\d+)/);
    if (m) round = { name: m[1], kept: +m[2], step: "starting", done: false, failed: false };
    if (round && l.includes("== ")) round.step = l.replace(/^.*== /, "");
    if (round && /pushed:/.test(l)) { round.done = true; round.step = "pushed"; target = round.kept + every; }
  ...
  return { start, deadline, target, round, nextRound: `v${n}`, finished: decided, tail: log.slice(-14) };
}
